# Xử lý video kết hợp với cảm biển siêu âm
...
flags.DEFINE_string('video', 'E:/Data/colision-warning/demo2/demo4_30.mp4', 'path to input video or set to 0 for webcam')
...
def main(_argv):
    # Lấy dữ liệu từ cảm biến siêu âm đã lưu trong file exel lưu vào `d`
    loc = ("18-6-2022.xlsx")
    ...
    while True:
        ...
        if frame_num % fps1 == 0:
            ...
            if current_time in d:
                distan = d[current_time]
                times_loop = 0
                print(f"distance: {distan}")
            else:
                times_loop += 1

        if times_loop < 7 and distan != -1:
            cv2.putText(frame, "Warning!", (width - 250, 150), 0, 1.5, (255, 215, 0), 3)
            cv2.putText(frame, str(distan) + " cm", (width - 250, 200), 0, 1.5, (255, 215, 0), 3)
        ...
        point1 = (int(original_w * 0.1), original_h)
        point3 = (int(original_w * 0.9), original_h)
        point2 = (int((point1[0] + point3[0])/2), int(original_h * 0.5))
