...
flags.DEFINE_string('video', 'E:/Data/colision-warning/test5.mp4', 'path to input video or set to 0 for webcam')
...
def main(_argv):
    # Đọc file vận tốc
    file1 = open(FLAGS.file_velocity, 'r')
    Lines = file1.readlines()
    
    v_list = []
    for line in Lines:
        v_list.append(int(line.strip()))
    ...
    while True:
        ...
        point1 = (int(original_w * 0.25), original_h)
        point3 = (int(original_w * 0.75), original_h)
        point2 = (int((point1[0] + point3[0])/2), int(original_h * 0.6))
        ...
        for track in tracker.tracks:
            ...
            cv2.putText(frame, str(round(distance, 1)) + "m", (int((bbox[0]+bbox[2])/2)-30, int((bbox[1]+bbox[3])/2)-10), 0, 0.75, (255, 255, 255), 2)
